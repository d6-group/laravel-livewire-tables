...
<div
    x-data="{ open: false, childElementOpen: false, isTailwind: @js($component->isTailwind()), isDaisyUI: @js($component->isDaisyUI()), isBootstrap: @js($component->isBootstrap()) }"
    x-cloak x-show="(selectedItems.length > 0 || alwaysShowBulkActions)"
    @class([
        'mb-3 mb-md-0' => $component->isBootstrap(),
        'w-full md:w-auto mb-4 md:mb-0' => ($component->isTailwind() || $component->isDaisyUI()),
    ])
>
    <div @class([
            'dropdown d-block d-md-inline' => $component->isBootstrap(),
            'relative inline-block text-left z-10 w-full md:w-auto' => ($component->isTailwind() || $component->isDaisyUI()),
        ])
    >
        <button
            @class([
                ...
                'inline-flex justify-center w-full btn btn-ghost btn-sm input-bordered font-medium' => $component->isDaisyUI(),
            ])
            type="button"
            id="{{ $tableName }}-bulkActionsDropdown" 
            
                        
            @if($component->isTailwind() || $component->isDaisyUI())
